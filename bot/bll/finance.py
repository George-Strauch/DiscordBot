...
from bot.services.finance_client import TickerInfo
...
class FinanceBll:
    ...
    def send_ticker_price(self, tickers: list, period: str = "1mo", return_price=False):
        ...
        if "error" in ticker_data:
            ...
            plt.close(picture_plot)

            with io.BytesIO() as image_binary:
                im_flipped.save(image_binary, format="PNG")
                image_binary.seek(0)
                embeds = [self._finance_embed(x, k) for k, x in ticker_data.items()]
                display = {
                    "embeds": embeds,
    ...
    def get_financial_statements(self, tickers: list, fields: list = None):
        try:
            ...
            logger.exception("Error getting financial statements")
            return {"errors": "errors occurred when getting information for the provided tickers from API"}
    ...
    def _finance_embed(self, ticker_data, t_name):
        ...
        e.set_footer(text="Source: YFinance")
        return e
    ...
    def _get_plt(self, tickers):
        ...
        for t, tdata in tickers.items():
            ...
            ax.plot(prices, linewidth=0.5, color=tdata["color"])
            base = [1] * len(prices)
            ax.plot(base, linewidth=0.1, color="#000000", alpha=0.2)
        ax.axis("off")
        return fig
